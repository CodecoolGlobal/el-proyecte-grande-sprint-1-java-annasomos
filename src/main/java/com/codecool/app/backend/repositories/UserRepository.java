...
public interface UserRepository extends JpaRepository<User, UUID>{
    ...
    "select * from app_user" +
           ...
           "where app_user.id = app_user_friends.friends_id"
     */

    @Query(value = "select * from app_user\n" +
            "left join app_user_friends on app_user.id =  app_user_friends.friends_id\n" +
            "where :app_user_friends.user_id = app_user_friends.user_id", nativeQuery = true)
    void getFriends(@Param("app_user_friends.user_id") UUID userId);
